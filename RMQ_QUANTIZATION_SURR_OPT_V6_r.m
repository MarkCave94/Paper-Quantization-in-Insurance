...
Gamma_X =S0+S0*r(g)*dt+S0*sigma*Gamma*sqrt(dt); %Gamma_x uno %
...
euro_call_BS_quant = exp(-r(g)*T)*Probabilita_successive_X(end,:)*max(Quantizzatori_X(:,end)-Strike,0);

[Call] = blsprice(S0, Strike, r(g), T, sigma);
...
min_gar_rate=max(  ((beta.*Fund_Returns)-i_tec)/(1+i_tec),  s_min);
...
Euro_Contract(g,1)= exp(-r(g)*T)*Probabilita_successive_X(1,:)*C_t(:,end); 
...
for k=(T):-1:1
    ...
    H_small_matrix_call(:,k)=max(H_small_matrix_call(:,k),exp(-r(g))*Transition_matrix*H_small_matrix_call(:,k+1));
   
end

AmerPrice(g,1)=exp(-r(g))*Probabilita_successive_X(1,:)*H_small_matrix_call(:,1);
...
writetable(Results,'Results_v6_r.xls')

AC_Input_data=importdata("AC_Input_data_r.xlsx");
...
title('American Contract varying r')
xlabel('r')
...
title('European Contract varying r')
xlabel('Beta')
...
title('Surrender Option varying r')
xlabel('r')
